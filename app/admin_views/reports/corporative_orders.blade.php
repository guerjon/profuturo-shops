...
{{Form::open([
  'id' => 'filter-form',
  'method' => 'GET',
  'action' => 'AdminApiController@getCorporationOrders',
  'target' => '_blank'
])}}
...
function update(){
  ...
  $.get('/admin/api/corporation-orders', $('#filter-form').serialize(), function(data){
    $('.table tbody').empty();
    if(data.status == 200){
  ...
  $('#filter-form *').change(function(){
    ...
    $(document).on('click', '.pagina', function(){
      ...
      $('#since').change(function(){
       ...
       $.ajax({
          url : '/admin/api/bi-corporation-autocomplete',
          dataType: 'json',
