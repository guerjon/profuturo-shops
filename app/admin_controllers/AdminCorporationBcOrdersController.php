...
class AdminCorporationBcOrdersController extends AdminBaseController{
  ...
  public function index()
  {
   if(Input::get('export') == 'xls'){
    ...
    $orders = DB::table('bc_orders')
      ->select('*','bc_orders.id as order_id','bc_orders.created_at as order_created_at')
      ->join('users','users.id','=','bc_orders.user_id')
      ->join('bc_order_business_card','bc_orders.id','=','bc_order_business_card.bc_order_id')
      ->join('business_cards','bc_order_business_card.business_card_id','=','business_cards.id')
      ->groupBy('bc_orders.id')
      ->orderBy('bc_orders.created_at', 'desc')
    ...
    if(Input::has('since'))
        $orders->where('bc_orders.created_at','>=',Input::get('since'));
    if(Input::has('to'))
      $orders->where('bc_orders.created_at','<=',Input::get('to'));


    return View::make('admin::corporation_bc_orders.index')->withBcOrders($orders->whereNull('bc_orders.deleted_at')->get());
  }
  ...
  public function show($bc_order_id)
  {
    ...
    return View::make('admin::corporation_bc_orders.show')->withBcOrder($bc_order)->withBlankCard($blank_card)->withExtra($extra);
  }
  ...
  public function destroy($bc_order_id)
  {
    ...
    return Redirect::to(action('AdminCorporationBcOrdersController@index'))->withSuccess('Se ha eliminado la orden');
  }
}
