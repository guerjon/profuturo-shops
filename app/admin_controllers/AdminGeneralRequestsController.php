			  ...
			  $excel->sheet('Solicitudes',function($sheet)use($request,$headers){
				...
				foreach ($general_requests as $request) {
					...
					$sheet->appendRow([
						...
						$presupuesto,
						$request->created_at ? $request->created_at->format('d-m-Y g:m A') : 'SIN FECHA',
						$request->project_date ? $request->project_date->format('d-m-Y g:m A') : 'SIN FECHA',
						$request->deliver_date ? $request->deliver_date->format('d-m-Y g:m A') : 'SIN FECHA',
