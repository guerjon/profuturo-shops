...
class CorporationBcOrdersController extends BaseController{

  	public function index()
  	{
		return View::make('corporation_bc_orders.index')->withBcOrders(Auth::user()->bc_orders);
  	}
  	...
  	public function show($bc_order_id)
  	{
		...
		if(!$bc_order){
		  	return Redirect::to(action('CorporationBcOrdersController@index'))->withErrors('No se encontró la orden');
		}
		if($bc_order->confirmed){
		  	$blank_card = DB::table('blank_cards_bc_order')->where('bc_order_id', $bc_order_id)->first();
		  	return View::make('corporation_bc_orders.show')->withBcOrder($bc_order)->withBlankCard($blank_card);
		}else{
		  	return Redirect::to(action('CorporationBcOrdersController@edit', [$bc_order->id]))->withInfo('Por favor, confirme los datos de las tarjetas para enviar la orden');
		}
  	...
  	public function postFillOrder()
  	{
		...
		if((count($cards) > 0) || (count(Input::get('talent',[])) > 0) || (count(Input::get('manager',[])) > 0) ){
			...
			return View::make('corporation_bc_orders/fill_order')
  	...
  	public function store()
  	{
		...
		return Redirect::to(action('CorporationBcOrdersController@index'))->withSuccess('Se ha guardado la orden satisfactoriamente');
  	}
  ...
  public function edit($bc_order_id){
   // $bc_order_id = $array["bc_order_id"]
	...
	return View::make('corporation_bc_orders.edit')
  ...
  public function update($bc_order_id)
  {
	...
	if($order->status == 0) 
	{
	  ...
	  return Redirect::to(action('CorporationBcOrdersController@index'))->withSuccess('Se ha eliminado la orden');  
	}else{
